...
function simplifyColors(gridItems, colorPallete, width, height) {
  ...
  for (let index = 0; index < colors.length; index++) {
    ...
    const newColorPallete = colorPallete * cmmdcForList(numbers);

    for (let i = 0; i < numbers.length - 1; i++) {
      numbers[i] = parseInt(numbers[i] / newColorPallete);
    }
  ...
  //     gridItems[i * width + j].backgroundColor = colors[i * width + j];
  //   }
  // }
}
